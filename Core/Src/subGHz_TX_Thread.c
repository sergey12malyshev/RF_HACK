...
{
    uint8_t status = 0;
    // TI_read_status(CCxxx0_VERSION); // it is for checking only //it must be 0x14
    ...
    DEBUG_PRINT(CLI_TX"%d"CLI_NEW_LINE, len);
...
PT_THREAD(subGHz_TX_Thread(struct pt *pt))
{
    ...
    while (1)
    {
        ...
        DEBUG_PRINT(CLI_TX"%s %d"CLI_NEW_LINE, packet, s);
